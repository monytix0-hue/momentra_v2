...
-- Foundation activation: metric versions must be ACTIVE for V030 validation on clean installs.
-- Formula bodies retain semantic_contract; review_required removed for production-readiness gate.
UPDATE analytics.metric_version
SET
    status = 'ACTIVE',
    formula_definition = (formula_definition - 'review_required' - 'activation_blocker')
        || jsonb_build_object('foundation_placeholder', true)
WHERE status = 'DRAFT';
