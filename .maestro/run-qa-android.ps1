...
<#
.SYNOPSIS
  S9-QA-A — Run Android Maestro suite by class (smoke|critical|isolation|all).
#>
param(
  [ValidateSet("smoke", "critical", "isolation", "all", "cert")]
  [string]$Class = "smoke",
  [string]$Device = "",
...
Write-Host "==> S9-QA class=$Class device=$target runId=$($env:MAESTRO_RUN_ID)"
...
if ($PrepareFixtures) {
  ...
  Pop-Location
  # Reload env after seed rewrote .env.maestro.local (keep current run id).
  $MaestroEnvArgs = Get-MaestroEnvCliArgs -EnvFile $EnvFile -RunId $RunId
}
...
$flowRoot = if ($Class -eq "cert") {
  Join-Path $Root ".maestro\cert\android"
} else {
  Join-Path $Root ".maestro\android"
}
if ($Class -eq "all") {
  & $Maestro --device $target test (Join-Path $Root ".maestro\android") @MaestroEnvArgs --format junit --output (Join-Path $reportDir "junit.xml") --debug-output $reportDir
} else {
  & $Maestro --device $target test $flowRoot @MaestroEnvArgs --include-tags $include --format junit --output (Join-Path $reportDir "junit.xml") --debug-output $reportDir
}

$maestroExit = $LASTEXITCODE

if ($VerifyBackend -or $Class -eq "cert") {
  Write-Host "==> Backend verify scaffold (qa:verify) - requires notes from run"
  Push-Location (Join-Path $Root "backend\typescript")
  $env:QA_FIXTURES_ENABLED = "true"
  $env:ALLOW_DEV_AUTH = "1"
...
Write-Host "==> Master Certification does NOT auto-PASS. See docs/qa/MASTER_QA_SUMMARY.md"
exit $maestroExit
