...
/// Figma 605:7780 (+ twins) — Shared Purchase Quick Add hub.
struct PurchaseQuickAddHubView: View {
    ...
    private var tiles: [PurchaseQuickAddKind] {
        ...
        let all = PurchaseQuickAddKind.hubTiles(theme: theme)
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Make every contribution count")
                            .font(.plusJakarta(size: 22, weight: .heavy))
                            .foregroundStyle(Color(hex: "#14121B"))
                        Text(
                            hasActiveMoment
                                ? "Add contributors, track expenses, plan purchases and coordinate delivery."
