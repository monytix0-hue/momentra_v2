...
/// Relationships Memory populated body — pink family; Memory GET honest empty (S2 G4).
struct PersonalRelationshipsMemoryActiveView: View {
    let refreshToken: UInt64
    let momentId: String?
    var onProtectConnection: () -> Void
    ...
    private var thinData: Bool {
        activities.count < 2 && PersonalRelationshipsDerived.bondIndex(pulse: pulse) == nil
    }
    ...
    private var identitySnapshot: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text(thinData ? "Connector in Motion" : "Bond Steward")
                .font(.system(size: 28, weight: .heavy))
                .foregroundStyle(Color(hex: "#E5E0EE"))
            Text(
                thinData
                    ? "Log connections and support to reveal your relationship identity."
                    : "You deepen bonds when presence stays intentional."
            )
            .font(.system(size: 14))
            .foregroundStyle(Color(hex: "#C9C4D8"))
        }
    ...
    private var memoryProjection: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("2 BOND AXES")
                ...
                .tracking(1.2)
                .foregroundStyle(Color(hex: "#C9C4D8"))
            axisRow("Trust", axes.trust)
            axisRow("Care", axes.care)
            axisRow("Support", axes.support)
            axisRow("Presence", axes.presence)
        }
        .padding(16)
        .frame(maxWidth: .infinity, alignment: .leading)
    ...
    private var corePattern: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("3 CORE PATTERN")
                ...
                .foregroundStyle(Color(hex: "#C9C4D8"))
            if thinData {
                Text("Building… Need a few more logs to show Connection → Support → Bond.")
                    .font(.system(size: 13))
                    .foregroundStyle(Color(hex: "#C9C4D8"))
            } else {
                Text("Connection → Support → Bond")
    ...
    private var growthEdge: some View {
        VStack(spacing: 10) {
            Text("Protect one weekly connection block")
                .font(.system(size: 16, weight: .heavy))
                .foregroundStyle(.white)
                .multilineTextAlignment(.center)
            Text("Schedule it before the next high-pressure stretch.")
                .font(.system(size: 13))
                .foregroundStyle(.white.opacity(0.85))
                .multilineTextAlignment(.center)
            Button(action: onProtectConnection) {
                Text("Log Connection Now")
    ...
    private var insightsCard: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text("Recurring themes across your Relationships journey will appear here.")
                .font(.system(size: 12))
                .foregroundStyle(Color(hex: "#C9C4D8"))
        }
        .padding(14)
