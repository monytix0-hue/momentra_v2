...
public typealias MomentraAPIAPIChallengeHandler = ((URLSession, URLSessionTask, URLAuthenticationChallenge) -> (URLSession.AuthChallengeDisposition, URLCredential?))
...
private var challengeHandlerStore = SynchronizedDictionary<Int, MomentraAPIAPIChallengeHandler>()
...
open class URLSessionRequestBuilder<T>: RequestBuilder<T> {
     ...
     */
    public var taskDidReceiveChallenge: MomentraAPIAPIChallengeHandler?
    ...
    @discardableResult
    override open func execute(_ apiResponseQueue: DispatchQueue = MomentraAPIAPI.apiResponseQueue, _ completion: @escaping (_ result: Swift.Result<Response<T>, ErrorResponse>) -> Void) -> RequestTask {
    ...
    open func buildHeaders() -> [String: String] {
        ...
        for (key, value) in MomentraAPIAPI.customHeaders {
        ...
        case is URL.Type:
            do {
                ...
                if let headerFileName = getFileName(fromContentDisposition: httpResponse.allHeaderFields["Content-Disposition"] as? String) {
                    ...
                    requestPath = requestPath.appending("/tmp.MomentraAPI.\(UUID().uuidString)")
                }
